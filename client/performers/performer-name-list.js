...
function update() {
    ...
    const additionsStr = additions.length ? `Added ${additions.map(a => chalk.bold(a)).join(', ')}` : 'No additions';
    const removalsStr = removals.length ? `removed ${removals.map(r => chalk.bold(r)).join(', ')}` : 'no removals';
