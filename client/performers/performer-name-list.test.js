...
beforeEach(() => {
  ...
  jest.mock(
    ...
    () => ({
      autocomplete: {
        performerNames: ["winnie.the.pooh", "piglet", "eeyore", "tigger"]
      }
    }),
    { virtual: true }
  );
...
test("add updates config", () => {
  ...
  add(["owl"]);
  expect(jsonWriterMock).toHaveBeenCalledWith(
...
test("add updates list", () => {
  const origPerformerNames = list();
  add(["owl"]);
  expect(list()).toEqual([...origPerformerNames, "owl"]);
});

test("add logs", () => {
  add(["owl"]);
  expect(logMock.log).toHaveBeenCalledWith(
    `Updated autocomplete set. Added ${chalk.bold("owl")}, no removals.`
  );
});
...
test("removes from list", () => {
  ...
  const performerNames = list();
  expect(performerNames).toEqual(["winnie.the.pooh", "tigger"]);
});
...
test("remove updates list", () => {
  remove(["winnie.the.pooh", "piglet", "eeyore"]);
  expect(list()).toEqual(["tigger"]);
});

test("remove logs", () => {
  remove(["piglet", "eeyore"]);
  expect(logMock.log).toHaveBeenCalledWith(
    `Updated autocomplete set. No additions, removed ${chalk.bold(
      "piglet"
    )}, ${chalk.bold("eeyore")}.`
  );
});
