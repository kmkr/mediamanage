...
const remote =
  config.mediaPlayer.remote ||
  `${process.env.SSH_CLIENT.split(" ")[0]}:${defaultPort}`;
const [hostname, port] = remote.split(":");

exports.play = filePath => {
  const wd = path.parse(filePath).dir;
  const fileName = path.parse(filePath).base;
  const url = `http://${hostname}:${port}/play?wd=${querystring.escape(
    wd
  )}&file=${querystring.escape(fileName)}`;
  http
    .get(url, res => {
      res.resume();
    })
