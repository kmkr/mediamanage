...
test("print paths as strings with sub dirs", () => {
  const paths = ["waz.mp4", "/wizzy/wozzy.mp4", "/bozzy/bazzy/wozzy.mp4"];
...
test("print paths as pairs", () => {
  const sourceFilePaths = ["/foo/bar/woz/waz/wiz.mp4", "/foo/baz/wizzy.mp4"];
  ...
  expect(calls[0][0]).toEqual(
    `${chalk.yellow("{..}bar/woz/waz")}/wiz.mp4 → ${chalk.green(
      "{..}me/mu/ma"
    )}/wiz.mp4`
  );
  expect(calls[1][0]).toEqual(
    `${chalk.yellow("        {..}baz")}/wizzy.mp4 → ${chalk.green(
      "{..}bo/be/baz/burb"
    )}/wizzy.mp4`
  );
});
