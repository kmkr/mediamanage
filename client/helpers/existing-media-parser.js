...
function match(filePath, categoriesAsString = false) {
  const [, title, performerNamesStr, categoriesStr] =
    filenameWithExt(filePath).match(STORED_FILE_REGEXP) || [];

  return {
    title,
    categories: categoriesAsString
      ? categoriesStr
      : parseCategoriesAsArrayStr(categoriesStr),
    performerNames: (performerNamesStr || "").split("_").filter(name => name)
  };
}
...
exports.renamePerformerName = ({ filePath, fromName, toName }) => {
  ...
  const newFileName = filenameWithExt(filePath).replace(
    STORED_FILE_REGEXP,
    match => {
      const performerNames = getPerformerNames(filePath);
      const newPerformerNames = performerNames.map(name =>
        name === fromName ? toName : name
      );

      return match.replace(
        `_${performerNames.join("_")}`,
        `_${newPerformerNames.join("_")}`
      );
    }
  );
