...
test("set title with categories and performers", () => {
  ...
  const actual = renamerHelper.setTitle("my.new.title", fileName);
  expect(actual).toBe(
    "(t:my.new.title)_(p:ole.brumm_nasse.noff)_(c:[foo][bar])__file.mp4"
  );
});

test("overwrite title with categories and performers", () => {
  const fileName =
    "(t:my.title)_(p:ole.brumm_nasse.noff)_(c:[foo][bar])__file.mp4";
  const actual = renamerHelper.setTitle("my.new.title", fileName);
  expect(actual).toBe(
    "(t:my.new.title)_(p:ole.brumm_nasse.noff)_(c:[foo][bar])__file.mp4"
  );
...
test("set performers", () => {
  const fileName = "(c:[woz])__file.mp4";
  const actual = renamerHelper.setPerformerNames(
    ["ole.brumm", "nasse.noff"],
    fileName
  );
...
test("clean file name with double underscore in folder name", () => {
  const fileName =
    "/woo/__waa/wii/(t:wobzy.bobzy)_(p:ole.b_noff)_(c:[wiz])__Wo wu, Bo by, Wi wa - ci fi [WoolB].mp4";
  const actual = renamerHelper.cleanFilePath(fileName);
