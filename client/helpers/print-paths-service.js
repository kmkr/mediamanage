...
exports.asPairsOfLists = ({ sourceFilePaths, destFilePaths }) => {
  ...
  assert(
    sourceFilePaths.length === destFilePaths.length,
    "Source and dest path length must be equal"
  );
  ...
  const longestSourceDir =
    getLongestDirLength(sourceFilePaths) -
    commonPart.length +
    commonPartPrefix.length -
    1;
  ...
  sourceFilePaths.forEach((sourcePath, index) => {
    ...
    const { dir: dDir, name: dName, ext: dExt } = path.parse(
      clean(destFilePaths[index])
    );
    logger.log(
      `${chalk.yellow(leftPad(clean(sDir), longestSourceDir))}${
        path.sep
      }${sName}${sExt} → ${chalk.green(clean(dDir))}${path.sep}${dName}${dExt}`
    );
  });
};
...
exports.asList = paths => {
  ...
  const longestDirLength = getLongestDirLength(
    paths.filter(p => !p.hidden).map(p => p.value || p)
  );
  ...
  paths.forEach((entry, index) => {
    ...
    if (!hidden) {
      logger.log(
        `${leftPad(index, indexLeftPad)}) ${leftPad(dir, longestDirLength)}${
          dir ? path.sep : " "
        }${chalk.green(base)}`
      );
    }
