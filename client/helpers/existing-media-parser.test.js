...
test("exclude performerNames not matching rules", () => {
  const fileName =
    "video.name_wab-Foozy_wAb-z_16.10.2017_16-10-2017_piglet.mp4";
  const performerNames = getPerformerNames(fileName);
  expect(performerNames).toEqual(["piglet"]);
});
...
test("rename performer name with full path, categories, index and multiple performer names", () => {
  const fileName =
    "/foo/bar/video.name_pigglet_winnie.the.pooh_[foo][bar]_(100).mp4";
  const newName = renamePerformerName({
    filePath: fileName,
    fromName: "pigglet",
    toName: "piglet"
  ...
  expect(newName).toEqual(
    "/foo/bar/video.name_piglet_winnie.the.pooh_[foo][bar]_(100).mp4"
  );
});
...
test("multi category extraction as strings", () => {
  expect(getCategories("foozzy_waz_[bar][foo].mp4", true)).toEqual(
    "[bar][foo]"
  );
});
