...
function move(sourceFilePath, destFileName) {
  ...
  const destFilePath = indexifyIfExists(path.resolve(destFileName));
...
exports.setPerformerNames = (performerNames, filePaths) => {
  assert(
    Array.isArray(performerNames),
    `Names must be an array. Was ${performerNames}`
  );
  assert(
  ...
  return filePaths.map(filePath => {
    const newFileName = getRenamerHelper(filePath).setPerformerNames(
      performerNames,
      filePath
    );
    return move(filePath, newFileName);
...
function setCategories(categories, filePath) {
  const sortedCategories = config.categories.filter(category =>
    categories.includes(category)
  );
  const newFileName = getRenamerHelper(filePath).setCategories(
    sortedCategories,
    filePath
  );
  return move(filePath, newFileName);
...
exports.setCategories = (categories, filePaths) => {
  assert(
    ...
    `Categories must be an array. Was ${categories}`
  );
  assert(
    Array.isArray(filePaths),
    `File paths must be an array. Was ${filePaths}`
...
exports.addCategories = (categories, filePaths) => {
  ...
  return filePaths.map(filePath => {
    const existingCategories = getRenamerHelper(filePath).getCategories(
      path.parse(filePath).base
    );

    setCategories(
      [...existingCategories, ...categories].filter(unique),
      filePath
    );
  });
};
